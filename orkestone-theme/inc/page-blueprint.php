 ...
 */
/**
 * Get vertical page config by WordPress page ID.
 *
 * Looks up the vertical JSON pages array for a page entry whose slug
 * matches the given WordPress page's post_name. Returns null if no
 * matching vertical page config is found.
 *
 * @param int $page_id WordPress page ID.
 * @return array|null Page config array with 'sections', or null.
 */
function vbb_get_vertical_page_by_id( $page_id ) {
	...
	// Get the page slug from WordPress.
	$slug = get_post_field( 'post_name', $page_id );
	...
	// Search vertical pages for a matching slug.
	foreach ( vbb_get_vertical_pages() as $page ) {
		$page_slug = isset( $page['slug'] ) ? sanitize_title( $page['slug'] ) : '';
