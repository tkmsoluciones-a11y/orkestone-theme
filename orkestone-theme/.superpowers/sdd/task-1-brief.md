...
**Project:** Vertical Block Base (Orkestone Theme) — Block Registry Refactor

## Context

This is the foundation task. All other tasks depend on this. The registry file becomes the single source of truth for all 19 block types. Baker functions, settings defaults, sanitization, and JS rendering all derive from this.

**Branch:** `feat/block-registry`

**Load order constraint:** Registry must be loaded BEFORE `block-baker.php`. Edit `functions.php` accordingly.

## Files

- **Create:** `inc/block-registry.php`
- **Modify:** `inc/block-baker.php` — REMOVE old `vbb_get_baker_map()` function (it conflicts with the one in registry)
- **Modify:** `inc/functions.php` — add `'inc/block-registry.php'` to `$vertical_block_base_files` array BEFORE `'inc/block-baker.php'`

## Interfaces Produced

- `vbb_get_block_registry(): array` — full 19-block registry
- `vbb_get_block_def(string $key): ?array` — single block definition
- `vbb_get_baker_map(): array` — maps block key → baker function name
- `vbb_get_block_defaults(): array` — default values per block
- `vbb_sanitize_block_data(string $key, array $data): array` — validates data against registry schema

## Requirements

### 1. Create `inc/block-registry.php`

Use the exact code below. The complete block registry defines ALL 19 blocks with their fields.

**Important PHP patterns used:**
- `'default' => value` — must match the existing data shapes in `admin-pro.js` renderers
- `'type' => 'image'` — marks fields for wp.media integration
- `'type' => 'repeatable'` with `'item_fields'` for repeatable items

### 2. Remove old `vbb_get_baker_map()` from `block-baker.php`

The existing file `inc/block-baker.php` at line 20 defines:
...
This MUST be removed (lines 15-35, the whole function), because the new `block-registry.php` defines `vbb_get_baker_map()` and `block-registry.php` loads first. Having both causes a fatal "Cannot redeclare" error.

**IMPORTANT:** The baker dispatcher `vbb_bake_section()` (line 45) calls `vbb_get_baker_map()` — that's fine, PHP resolves to the one in registry.

### 3. Update `functions.php` load order

In `orkestone-theme/functions.php`, insert `'inc/block-registry.php'` BEFORE `'inc/block-baker.php'` in the `$vertical_block_base_files` array:
...
- `php -l inc/block-registry.php` — no syntax errors
- `php -l inc/block-baker.php` — no syntax errors (after removing old function)
- No "Cannot redeclare" errors when loading WordPress
- `vbb_get_block_def('hero')` returns the hero block definition
- `vbb_get_baker_map()` returns all 19+ keys (12 old + 7 new + divider)

---

## Complete Code: `inc/block-registry.php`
...
After completing:
1. Write a report to `.superpowers/sdd/task-1-report.md`
2. Run `php -l inc/block-registry.php` and `php -l inc/block-baker.php` and include results
3. Include a `git log --oneline -3` showing your commits
4. Include a one-line test summary
