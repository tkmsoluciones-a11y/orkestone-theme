# Settings Cache Specification

## Purpose

Wrap the expensive `vbb_pro_sanitize_settings()` path with a WordPress Transient layer so resolved settings are cached between admin saves. Covers the `settings-cache` capability introduced by the builder-performance-optimization change.
