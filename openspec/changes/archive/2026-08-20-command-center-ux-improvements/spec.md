...
### Acceptance Criteria
- [ ] Status bar displays `vbb-cc-status-bar--unsaved` when `dirty === true` and no save is in progress
- [ ] Status bar reverts to idle/saved after successful save
- [ ] Tooltip "Unsaved changes" appears on status bar when dirty
- [ ] Modified inputs get `vbb-saved-flash` animation
- [ ] All existing save behaviors remain unchanged
...
### Acceptance Criteria
- [ ] Undo stack limited to 5 entries (color/palette changes only)
- [ ] Undo button is disabled when stack is empty
- [ ] Redo button is disabled when redo stack is empty
- [ ] Clicking Undo reverts the last color change
- [ ] Clicking Redo re-applies the undone change
- [ ] Undo/Redo state is per-session only (refresh page → reset)
- [ ] Non-color changes do not affect the undo stack
...
### Acceptance Criteria
- [ ] Toolbar "Compare" button toggles comparison mode
- [ ] Default mode is `'after'` (current live preview)
- [ ] Comparison mode persists via localStorage
- [ ] `'before'` mode shows saved baseline CSS vars in preview
- [ ] `'after'` mode shows current changes in preview
- [ ] Mode is remembered across page reloads
...
### Acceptance Criteria
- [ ] "Export Profile" button downloads JSON with current settings
- [ ] "Import Profile" button opens file input
- [ ] Valid JSON profiles are applied successfully
- [ ] Invalid JSON shows error toast
- [ ] Existing "Save as Profile" functionality remains intact
- [ ] Export includes: profileName, colorMode, palettes (light/dark), typography, layout, blocks, buttons, exportedAt, theme, profileType, schemaVersion
...
### Acceptance Criteria
- [ ] Ctrl+S triggers save, shows saving/saved/error states correctly
- [ ] Ctrl+Z triggers undo when actions available, shows appropriate toast
- [ ] Ctrl+Z shows "No actions to undo" when stack empty
- [ ] Shortcuts disabled when focus is on input fields
- [ ] Keyboard shortcut indicator displayed in UI
- [ ] Default browser save behavior (Ctrl+S → save page) is prevented
