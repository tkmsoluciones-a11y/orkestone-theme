...
function vbb_get_active_vertical_settings() {
	...
	return array(
		'active'   => '' !== $option_active ? sanitize_key( $option_active ) : ( ! empty( $settings['active'] ) ? sanitize_key( $settings['active'] ) : 'default' ),
		'fallback' => ! empty( $settings['fallback'] ) ? sanitize_key( $settings['fallback'] ) : 'default',
	);
}
